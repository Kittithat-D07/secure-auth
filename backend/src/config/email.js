...
const sendOTPEmail = async (to, name, code, type = "verify") => {
  ...
  try {
    ...
    console.log("✅ OTP email sent to:", to, "| Brevo messageId:", res.data?.messageId);
  } catch (err) {
    console.error("❌ Brevo sendOTPEmail error:", JSON.stringify(err.response?.data || err.message));
    throw err;
  }
};
...
const sendResetPasswordEmail = async (to, name, resetUrl) => {
  try {
    ...
    console.log("✅ Reset email sent to:", to, "| Brevo messageId:", res.data?.messageId);
  } catch (err) {
    console.error("❌ Brevo sendResetPasswordEmail error:", JSON.stringify(err.response?.data || err.message));
    throw err;
  }
};
